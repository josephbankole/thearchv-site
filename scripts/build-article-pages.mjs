...
/* ---------- the sources line: named outlets become links ----------
...
function linkSources(escapedText) {
...
const isSourcesPara = (p) => /^Sources?\s*:/i.test(p);
...
function bodyHtml(text) {
  return String(text)
    ...
    .map((p) => p.trim())
    .filter(Boolean)
    .map((p) => (isSourcesPara(p) ? `<p class="article__sources">${linkSources(esc(p))}</p>` : `<p>${esc(p)}</p>`))
    .join("\n        ");
}

// tests-by-assertion, same convention as metaDescription below: a regression fails the build.
(function selfTestSourceLinks() {
  const out = bodyHtml("Sources: Sky Sports; ESPN and Sky Sports again; David Ornstein; Manchester United official.");
  ...
  const body = bodyHtml("Sky Sports report that Manchester United moved.\n\nSources: Sky Sports.");
